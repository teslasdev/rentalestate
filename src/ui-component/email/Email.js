...
const Emails = ({ toggleVisibilityEmail, isVisibleEmail }) => {
  ...
  return (
    <div>
      {/* Add email */}
      <Addemail open={openEmail} handleClose={handleCloseEmail} />
        ...
        <Grid container display="flex" alignItems="center">
          <Stack direction="row" alignItems="center" justifyContent={'space-between'} width={'100%'}>
            ...
            <Stack direction="row" alignItems="center" justifyContent={'flex-end'} spacing={2}>
              {isVisibleEmail && (
                <Button variant="contained" color="secondary" size="small" startIcon={<AddIcon />} onClick={handleOpenEmail}>
                  Create Email
                </Button>
              )}
            </Stack>
      ...
      {isVisibleEmail && (
        <TableStyleTwo>
          <Box width="100%" height="30vh">
            <DataGrid rows={rows} columns={columns} columnHeaderHeight={40} getRowId={(row) => row._id} />
          </Box>
        </TableStyleTwo>
      )}
    </div>
