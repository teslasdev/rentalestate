...
const SelectPropertyImagesDialog = ({ open, onClose }) => {
    ...
    setSelectedFiles((prevFormData) => {
      ...
      Array.from(files).forEach((file, index) => {
        if (file.type.startsWith('image/')) {
          ...
          toast.error('Property Photo should be an image');
  ...
  const handleAddImages = async () => {
    try {
      ...
      let response = await postApi(`api/property/add-property-photos/${propertyId}`, selectedFiles);
      if (response.status === 200) {
        toast.success('Property Photo Added successfully');
        navigate(0);
        setSelectedFiles(new FormData()); // Reset FormData after successful upload
      } else {
        toast.error('Cannot add Property Photo');
      }
    } catch (e) {
      console.error(e);
      toast.error('Cannot add Property Photo');
