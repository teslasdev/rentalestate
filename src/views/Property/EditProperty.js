...
const AddProperty = (props) => {
  ...
  const initialValues = {
    ...
    parkingAvailability: data?.parkingAvailability || 'default',
    appliancesIncluded: data?.appliancesIncluded,
    heatingAndCoolingSystems: data?.heatingAndCoolingSystems,
    flooringType: data?.flooringType,
    exteriorFeatures: data?.exteriorFeatures,

    listingStatus: data?.listingStatus || 'default',
    listingAgentOrTeam: data?.listingAgentOrTeam,
    listingDate: moment(data?.listingDate).format('YYYY-MM-DD'),
    marketingDescription: data?.marketingDescription,
    multipleListingService: data?.multipleListingService,
    previousOwners: data?.previousOwners,

    propertyTaxes: data?.propertyTaxes,
    homeownersAssociation: data?.homeownersAssociation,
    mortgageInformation: data?.mortgageInformation,
    sellers: data?.sellers,
    buyers: data?.buyers,
    propertyManagers: data?.propertyManagers,
    contractorsOrServiceProviders: data?.contractorsOrServiceProviders,
    internalNotesOrComments: data?.internalNotesOrComments
  };

  const AddData = async (values, resetForm) => {
    try {
      values.createBy = JSON.parse(localStorage.getItem('user'))._id;
      let response = await patchApi(`api/property/edit/${data._id}`, values);
      if (response.status === 200) {
        toast.success('Property Modified successfully');
        handleClose();
        // resetForm();
      } else {
        toast.error('cannot modify property');
      }
    } catch (e) {
      console.log(e);
      toast.error('cannot modify property');
