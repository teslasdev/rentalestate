...
const EditLead = (props) => {
  ...
  const initialValues = {
    ...
    leadPhoneNumber: data?.leadPhoneNumber,
    leadAddress: data?.leadAddress,
    // Lead Source and Details:
    leadSource: data?.leadSource,
    leadStatus: data?.leadStatus || 'default',
    leadSourceDetails: data?.leadSourceDetails,
    leadCampaign: data?.leadCampaign,
    leadSourceChannel: data?.leadSourceChannel,
    leadSourceMedium: data?.leadSourceMedium,
    leadSourceCampaign: data?.leadSourceCampaign,
    leadSourceReferral: data?.leadSourceReferral,
    // Lead Assignment and Ownership:
    leadAssignedAgent: data?.leadAssignedAgent,
    leadOwner: data?.leadOwner,
    leadCommunicationPreferences: data?.leadCommunicationPreferences,
    // Lead Dates and Follow-up:
    leadCreationDate: moment(data?.leadCreationDate).format('YYYY-MM-DD'),
    leadConversionDate: moment(data?.leadConversionDate).format('YYYY-MM-DD'),
    leadFollowUpDate: moment(data?.leadFollowUpDate).format('YYYY-MM-DD'),
    leadFollowUpStatus: data?.leadFollowUpStatus,
    // Lead Scoring and Nurturing:
    leadScore: data?.leadScore,
    leadNurturingWorkflow: data?.leadNurturingWorkflow,
    leadEngagementLevel: data?.leadEngagementLevel,
    leadConversionRate: data?.leadConversionRate,
    leadNurturingStage: data?.leadNurturingStage,
    leadNextAction: data?.leadNextAction
  };

  const editLeadData = async (values) => {
    try {
      // setIsLoding(true);
      let response = await patchApi(`api/lead/edit/${data._id}`, values);
      if (response.status === 200) {
        toast.success('Lead Modified successfully');
        formik.resetForm();
      } else {
        toast.error('cannot edit lead');
      }
    } catch (e) {
      toast.error('cannot edit lead');
      console.log(e);
